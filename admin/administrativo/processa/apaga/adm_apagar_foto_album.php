<?php
	...
	$select_blog_fotos = "SELECT * FROM fotos_blog_estados WHERE id = '$id_foto'";
	$resultado_blog_fotos = mysqli_query($conn, $select_blog_fotos);
	$exibe_blog_fotos = mysqli_fetch_assoc($resultado_blog_fotos);
	
	//seleciona linha do banco
	$select_blog = "SELECT * FROM blog_estados WHERE id = '$id_album'";
	$resultado_blog = mysqli_query($conn, $select_blog);
	$exibe_blog = mysqli_fetch_assoc($resultado_blog);
	
	$pasta_caminho_todas_fotos = '../../../../imgs-sistema/img-estados/'.$exibe_blog['blog_album'].'/'.$exibe_blog_fotos['foto'];
...
<html lang="pt-br">
	...
	<body> <?php
		if($exibe_blog_fotos != 0){
			...
			$result_blog_album = "DELETE FROM  fotos_blog_estados WHERE id = '$id_foto'";
			$resultado_apagar = mysqli_query($conn, $result_blog_album);
			echo "
				
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../../../administrativo.php?link=24&id=".$id_blog_estados."''>
