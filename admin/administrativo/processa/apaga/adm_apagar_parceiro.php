<?php
	...
	$select_produto = "SELECT * FROM parceiros WHERE id = '$id'";
	$resultado_produto = mysqli_query($conn, $select_produto);
	$exibe_produto = mysqli_fetch_assoc($resultado_produto);
	
	$pasta_caminho = '../../../../imagem_geral/parceiro/'.$exibe_produto['foto_parceiro'];
	//echo "$pasta_caminho";
...
<html lang="pt-br">
	...
	<body> <?php
		if($exibe_produto != 0){
			...
			$result_produtos = "DELETE FROM parceiros WHERE id = '$id'";
			$resultado_apagar = mysqli_query($conn, $result_produtos);
			echo "
				
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../../../administrativo.php?link=30'>
