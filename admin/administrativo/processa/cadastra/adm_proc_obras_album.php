<?php
	...
	$id_obras = mysqli_real_escape_string($conn, $_POST['id']);
	$obras_album = mysqli_real_escape_string($conn, $_POST['obras_album']);
	
	
	//cria a pasta onde serão colocados os arquivos
		//Pasta onde o arquivo vai se salvo
		$_UP['pasta'] = '../../../../imgs-sistema/img-obras/'.$obras_album.'/';
		...
		if(array_search($extensao, $_UP['extensoes']) === false){	
			 echo "
				...
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../../../administrativo.php?link=49&id=".$id_obras."'>
		...
		elseif($_UP['tamanho'] < $_FILES['arquivo']['size']){	
			 echo "
				...
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../../../administrativo.php?link=48&id=".$id_obras."'>
		...
		//O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta arquivo
			...
			if(move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'].$nome_final)){
				...
				$result_produtos = "INSERT INTO fotos_obras (id_obras, foto) VALUES ('$id_obras', '$nome_final')";
